...
int insertarInt(tLista *p, char* lexema) 
{
    ...
    result = insertarEnOrden(p, nombreInsertar, "Const_Int", lexema, 0);
    ...
    if(result == DUPLICADO)
        {
            printf("La constante int %s ya se ingreso en la tabla de simbolos\n",lexema);
            return DUPLICADO;
        }
...
int insertarFloat(tLista *p, char* lexema) 
{
    ...
    /* El tipo no se muestra en CTES en la TS. Por eso en lugar del parámetro tipo ponemos "" */
    result = insertarEnOrden(p, nombreInsertar, "Const_Float" , lexema, 0);

    /* Para el caso de int, float y string, podemos tener una misma CTE y eso no significa que esté mal, solamente que se repite en el código y debemos almacenarla una sola vez */
    if(result == DUPLICADO)
        {
             printf("La constante float %s ya se ingreso en la tabla de simbolos\n",lexema);
            return DUPLICADO;
        }
...
int insertarString(tLista *p, char* lexema)
{
    ...
    /* El tipo no se muestra en CTES en la TS. Por eso en lugar del parámetro tipo ponemos "" */
    result = insertarEnOrden(p, nombreInsertar, "Const_String", valorInsertar, strlen(valorInsertar));
